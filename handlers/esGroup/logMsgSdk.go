...
func LogMessageSdk(gaugeIns *prometheus.GaugeVec, req *prometheus.Registry) http.Handler {
	...
	logs.Infoln("从配置文件config_es_expoter获取到的配置:", esExporterConfig.LogMessageAppName)
	// 获取sync个数

	for _, item := range esExporterConfig.LogMessageAppName {
		Urls := item.Urls
		cycle := item.Cycle
		...
		for _, url := range Urls {
			...
			for _, metricName := range metricNames {
				wg.Add(1)
				go GetEsSdkData(gaugeIns, esClient, gteTime, lteTime, metricName, url, cycle, wg.Done)
			}
		}
	}
	wg.Wait()
...
func GetEsSdkData(gaugeIns *prometheus.GaugeVec, esClient *elastic.Client, gteTime time.Time, lteTime time.Time,
